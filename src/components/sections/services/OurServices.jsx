    ...
    <div className="flex flex-col gap-7 max-h-[85vh] overflow-auto sticky top-[20%] p-7 lg:p-0">
        ...
        <Section title="Categories" open={openSections.category} toggle={() => toggleSection("category")}>
            <ul className="space-y-2">
                <li
                    className={`cursor-pointer px-4 py-2 rounded-full ${!selectedCategory && "border border-footer text-footer"}`}
        ...
        <Section title="Sub Categories" open={openSections.subcategory} toggle={() => toggleSection("subcategory")}>
            <ul className="space-y-2">
                <li
                    className={`cursor-pointer px-4 py-2 rounded-full ${!selectedSubcategory && "border border-footer text-footer"}`}
...
export default function OurServices() {
    ...
    const subcategories = useMemo(
        () => [...new Set(services.map(s => s.subcategory?.name).filter(Boolean))],
        [services]
    );
    ...
    return (
        <section className="wrapper py-10 flex gap-[2vw]">
            {/* Desktop Sidebar */}
            ...
            <div className="lg:w-[80%]">
                ...
                {filteredServices.map((service, index) => (
                    <div
                        key={service.id}
                        className={`bg-[#F6F6F6] rounded-2xl p-5 mb-10 flex flex-col lg:flex-row gap-6
